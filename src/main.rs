...
//	}
	...
	for _ in 0..num_cores {vals.push(Vec::with_capacity(batch_size / num_cores))}
	...
	for (x, y) in xs.iter().zip(ys.iter()) {
		if i < batch_size {
			...
			let g = crossbeam::scope(|scope| {
				...
				let mut it = handles.drain(0..num_cores);
...
fn gen_vec(n: usize) -> Vec<f32> {
	...
	for _ in 0..n { r.push(rng.gen_range(-0.05,0.05)); }
	r
} 
